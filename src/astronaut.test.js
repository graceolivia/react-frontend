...
test("convertJsonToAstronauts handles when there are multiple astronauts", () => {
  const jsonString =
    '{"people": [{"craft": "Rocketship", "name": "David Bowie"}, {"craft": "Spaceship", "name": "Lance Armstrong"}], "message": "success", "number": 2}';
  const jsonObject = JSON.parse(jsonString);
  const astronautArray = convertJsonToAstronauts(jsonObject);
  const expectedArray = [
    new Astronaut("Rocketship", "David Bowie"),
    new Astronaut("Spaceship", "Lance Armstrong"),
  ];
  expect(astronautArray).toEqual(expectedArray);
});
